...
@app.get("/movement/{id_mov}", response_model=schemas.Movement)
...
@app.post("/movement/", response_model=schemas.Movement, status_code=status.HTTP_201_CREATED)
...
### PATCH ###
@app.patch("/movement/{id_mov}", response_model=schemas.Movement)
    ...
    if id_product != None:
        ...
        try:
            ...
            if quantity_change != None:
                db_product_new_mov.quantity += quantity_change
                db_mov.quantity_change = quantity_change
            else:
                db_product_new_mov.quantity += db_mov.quantity_change
